...
inline wstring HostFileName()
{
	wchar_t strPath[MAX_PATH];
	GetModuleFileNameW(NULL, strPath, MAX_PATH);
	return wstring(PathFindFileNameW(strPath));
}

inline wstring DLLFileName(HINSTANCE hModule)
{
	wchar_t strPath[MAX_PATH];
...
inline wstring const DXErrStr(HRESULT dierr)
{
...
const string GUIDtoStringA(const GUID g);
const wstring GUIDtoStringW(const GUID g);

void StringToGUID(LPCWSTR szBuf, GUID *rGuid);
void StringToGUID(LPCSTR szBuf, GUID *rGuid);
