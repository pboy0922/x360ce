        ...
        if(iHookThis->GetHookMode() & iHook::HOOK_DI)
        {
            for(size_t i = 0; i < iHookThis->GetHookCount(); i++)
            {
				...
				if(padconf.GetHookState() && IsEqualGUID(padconf.GetProductGUID(),pInst->guidProduct))
                {
					...
					string strTrueguidProduct = GUIDtoStringA(pInst->guidProduct);
					string strHookguidProduct = GUIDtoStringA(HookInst.guidProduct);
					...
					if(iHookThis->GetHookMode() & iHook::HOOK_NAME)
                    {
						...
						string OldProductName = HookInst.tszProductName;
						string OldInstanceName = HookInst.tszInstanceName;
