...
void ReadConfig()
{
	...
	else
	{
		...
		else
		{
			...
			if (hookCheck) g_Hooks.EnableHook(iHook::HOOK_LL);

			hookCheck = ini.get_bool("InputHook", "HookCOM");
			if (hookCheck) g_Hooks.EnableHook(iHook::HOOK_COM);

			hookCheck = ini.get_bool("InputHook", "HookDI");
			if (hookCheck) g_Hooks.EnableHook(iHook::HOOK_DI);

			hookCheck = ini.get_bool("InputHook", "HookPIDVID");
			if (hookCheck) g_Hooks.EnableHook(iHook::HOOK_PIDVID);

			hookCheck = ini.get_bool("InputHook", "HookSA");
			if (hookCheck) g_Hooks.EnableHook(iHook::HOOK_SA);

			hookCheck = ini.get_bool("InputHook", "HookNAME");
			if (hookCheck) g_Hooks.EnableHook(iHook::HOOK_NAME);

			hookCheck = ini.get_bool("InputHook", "HookSTOP");
			if (hookCheck) g_Hooks.EnableHook(iHook::HOOK_STOP);

			hookCheck = ini.get_bool("InputHook", "HookWT");
			if (hookCheck) g_Hooks.EnableHook(iHook::HOOK_WT);

			hookCheck = ini.get_bool("InputHook", "HookNoTimeout");
			if (hookCheck) g_Hooks.EnableHook(iHook::HOOK_NOTIMEOUT);
	...
	if (g_Hooks.GetState(iHook::HOOK_PIDVID))
	{
		...
		if (vid != 0x045E || pid != 0x28E) g_Hooks.SetFakePIDVID(MAKELONG(vid, pid));
	}
