...
bool XInputInitialize()
{
	...
	bHookLL = g_Hooks.GetState(iHook::HOOK_LL);
	if (bHookLL) g_Hooks.DisableHook(iHook::HOOK_LL);

	PrintLog("Loading %ls", str.c_str());
	xinput.dll = LoadLibraryW(str.c_str());
	if (bHookLL) g_Hooks.EnableHook(iHook::HOOK_LL);
...
extern "C" DWORD WINAPI XInputGetState(DWORD dwUserIndex, XINPUT_STATE* pState)
{
	...
	if (mapping.DpadPOV > 0 && mapping.PovIsButton == false)
	{
		...
		if (povdeg >= 0)
		{
			...
			if (is_in_range_in(povdeg, mapping.pov[GAMEPAD_DPAD_LEFT] + 1, mapping.pov[GAMEPAD_DPAD_UP]) || is_in_range_in(povdeg, 0, mapping.pov[GAMEPAD_DPAD_RIGHT] - 1))
				xstate.Gamepad.wButtons |= XINPUT_GAMEPAD_DPAD_UP;

			// Up-right, right, down-right
			if (is_in_range_out(povdeg, 0, mapping.pov[GAMEPAD_DPAD_DOWN]))
				xstate.Gamepad.wButtons |= XINPUT_GAMEPAD_DPAD_RIGHT;

			// Down-right, down, down-left
			if (is_in_range_out(povdeg, mapping.pov[GAMEPAD_DPAD_RIGHT], mapping.pov[GAMEPAD_DPAD_LEFT]))
				xstate.Gamepad.wButtons |= XINPUT_GAMEPAD_DPAD_DOWN;

			// Down-left, left, up-left
			if (is_in_range_out(povdeg, mapping.pov[GAMEPAD_DPAD_DOWN], mapping.pov[GAMEPAD_DPAD_UP]))
...
extern "C" DWORD WINAPI XInputSetState(DWORD dwUserIndex, XINPUT_VIBRATION* pVibration)
{
	...
	PrepareForce(device, 0);
	PrepareForce(device, 1);

	if (!XInputIsEnabled.bEnabled && XInputIsEnabled.bUseEnabled)
	{
		SetDeviceForces(device, 0, 0);
		SetDeviceForces(device, 0, 1);
		return ERROR_SUCCESS;
	}
	...
	hr = SetDeviceForces(device, wLeftMotorSpeed, 0);

	if (FAILED(hr))
		PrintLog("SetDeviceForces for pad %d failed with code HR = %X", dwUserIndex, hr);

	hr = SetDeviceForces(device, wRightMotorSpeed, 1);
