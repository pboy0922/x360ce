...
HRESULT InitDirectInput(HWND hDlg, DInputDevice& device)
{
	...
	bHookDI = g_Hooks.GetState(iHook::HOOK_DI);
	bHookSA = g_Hooks.GetState(iHook::HOOK_SA);

	if (bHookDI) g_Hooks.DisableHook(iHook::HOOK_DI);
	if (bHookSA) g_Hooks.DisableHook(iHook::HOOK_SA);
	...
	if (bHookSA) g_Hooks.EnableHook(iHook::HOOK_SA);
	if (bHookDI) g_Hooks.EnableHook(iHook::HOOK_DI);
...
HRESULT PrepareForceNew(DInputDevice& device, bool motor)
{
	if (!device.ff.effect[motor])
	{
		...
		if (motor == 0) geff = GUID_SawtoothDown;
		if (motor == 1) geff = GUID_SawtoothUp;
